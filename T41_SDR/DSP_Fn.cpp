...
FLASHMEM void AGCLoadValues() {
  ...
  max_gain = powf (10.0, (float32_t)bands[t41.ActiveBand].agcThresh / 20.0);
...
void AGC() {
  ...
  if(AGCMode == 0)  // AGC OFF
  {
    ...
    {
      audioIFFT[512 + 2 * i + 0] = fixed_gain * audioIFFT[512 + 2 * i + 0];
      audioIFFT[512 + 2 * i + 1] = fixed_gain * audioIFFT[512 + 2 * i + 1];
    }
    return;
  }
  ...
  for(unsigned i = 0; i < 256; i++)
  {
    ...
    ring[2 * in_index + 0] = audioIFFT[512 + 2 * i + 0];
    ring[2 * in_index + 1] = audioIFFT[512 + 2 * i + 1];
    ...
    audioIFFT[512 + 2 * i + 0] = out_sample[0] * mult;
