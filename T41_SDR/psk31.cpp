...
void ProcessPSK31WaveData() {
  ...
  if(readWave(audioBufferR, 85 + 1)) { // 256 / 3 + 1
    ...
    for(unsigned int i = 1; i < 85; i++) {
      audioBufferL[3*i-2] = (audioBufferR[i-1] + audioBufferR[i]) / 3.0;
      audioBufferL[3*i-1] = (audioBufferR[i-1] + audioBufferR[i]) * 2.0 / 3.0;
      audioBufferL[3*i] = audioBufferR[i];
    }
    wold = audioBufferR[85];
    ...
    switch(0) {
      case 0: // I/Q
        ...
        arm_fir_f32(&FIR_Hilbert_L, audioBufferL, audioBufferL_EX, 256);
        arm_fir_f32(&FIR_Hilbert_R, audioBufferR, audioBufferR_EX, 256);
        //arm_fir_f32(&FIR_Hilbert_R, audioBufferL, audioBufferR_EX, 256);

        // Prepare psk decoder input buffer
        // use audioFFT (left channel: re, right channel: im)
        // we'll decode this in the demod section
        for(int i = 0; i < 256; i++) {
          audioFFT[512 + i * 2] = audioBufferL_EX[i]; // real
          audioFFT[512 + i * 2 + 1] = audioBufferR_EX[i]; // imaginary
        }
        break;
      ...
      case 1: // csdr library dbpsk_decoder_c_u8
        ...
        if(dataLoop == 0) {
          audioFFT[dataIndex] = audioBufferR[0]; // real
          audioFFT[dataIndex + 1] = 0; // imaginary
          dataLoop = 3;
          dataIndex += 2;
        }
        dataLoop--;
        ...
        if(dataIndex == 256) {
          ...
          dbpsk_decoder_c_u8(audioFFT, psk31Buffer, 256);
