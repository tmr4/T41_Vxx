...
static void waterfall_init(ftx_waterfall_t* me, int max_blocks, int num_bins, int time_osr, int freq_osr)
{
    ...
    me->mag = (WF_ELEM_T*)extmem_malloc(mag_size);
...
void monitor_init(monitor_t* me, const monitor_config_t* cfg)
{
    ...
    me->window = (float*)extmem_malloc(me->nfft * sizeof(me->window[0]));
    ...
    me->fft_work = extmem_malloc(fft_work_size);
    ...
    me->ifft_work = extmem_malloc(ifft_work_size);
...
void monitor_free(monitor_t* me)
{
    ...
    extmem_free(me->fft_work);
    extmem_free(me->last_frame);
    extmem_free(me->window);
}
...
void monitor_process(monitor_t* me, const float* frame)
{
    ...
    if (me->wf.num_blocks >= me->wf.max_blocks) {
      LOG(LOG_ERROR, "ERROR: waterfall blocks, %d, is greater than allowed, %d\n", me->wf.num_blocks, me->wf.max_blocks);
      return;
    }
    ...
    for (int time_sub = 0; time_sub < me->wf.time_osr; ++time_sub)
    {
        ...
        kiss_fft_scalar *timedata = (kiss_fft_scalar *)extmem_malloc(me->nfft * sizeof(kiss_fft_scalar));
        //kiss_fft_cpx freqdata[me->nfft / 2 + 1];
        kiss_fft_cpx *freqdata = (kiss_fft_cpx *)extmem_malloc((me->nfft / 2 + 1) * sizeof(kiss_fft_cpx));
        ...
        extmem_free(timedata);
        extmem_free(freqdata);
    }
