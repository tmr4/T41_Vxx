...
void SetFreq(int freq, bool reset) {
  ...
  if(t41.RadioState == CW_TRANSMIT_STRAIGHT_STATE || t41.RadioState == CW_TRANSMIT_PADDLE_STATE || t41.RadioState == CW_TRANSMIT_KEYER_STATE) {
    if(t41.DemodMode == DEMOD_LSB) {
      shift = CWFreqShift;  // flip CWFreqShift, sign originally minus
    } else {
      if(t41.DemodMode == DEMOD_USB) {
        shift = -CWFreqShift;  //  flip CWFreqShift, sign originally plus
      }
    }
  }
  Clk1SetFreq = (freq + shift) * SI5351_FREQ_MULT * MASTER_CLK_MULT;
  ...
  if(t41.RxTxState == RX_STATE) {   //  Receive state
  ...
  if(t41.RxTxState == TX_STATE) {  // Transmit state
