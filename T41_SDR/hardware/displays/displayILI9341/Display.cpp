...
// Current draw for T41 mock up on Prototyping System, amps (IlI9341 320x280)
// 3.2" 0.267
// 2.8" 0.272
// 2.4" 0.297
//ILI9341_t3 tft = ILI9341_t3(TFT_CS, TFT_RST, TFT_MOSI, TFT_SCLK, TFT_MISO);
...
//#endif
  ...
  tft.setRotation(3); // connector on right
...
FASTRUN void DrawFreqSpectrum(bool newSpectrumFlag /* = false */) {
  ...
  for(int x1 = 0; x1 < SPECTRUM_RES - 1; x1++) {
    ...
    if(eraseSpec && (displayState == DISPLAY_T41)) {
      tft.drawLine(SPECTRUM_LEFT_X + x1, yOldPlot[x1 + 1], SPECTRUM_LEFT_X + x1, yOldPlot[x1], ILI9341_BLACK);
    }
    ...
    if(drawSpec && (displayState == DISPLAY_T41)) {
      tft.drawLine(SPECTRUM_LEFT_X + x1, y1Plot, SPECTRUM_LEFT_X + x1, yPlot, ILI9341_YELLOW);
    }
...
FLASHMEM void ShowSpectrumFreqValues() {
  ...
  tft.print(txt);
  //tft.drawFastVLine(SPECTRUM_LEFT_X + centerLine - tickX, SPEC_BOX_LABELS - 4, 6, ILI9341_YELLOW);
  tft.drawFastVLine(centerLine - tickX, SPEC_BOX_LABELS - 8, 6, ILI9341_YELLOW);
...
FLASHMEM void ShowOperatingStats() {
  ...
  //tft.fillRect(OPERATION_STATS_L, OPERATION_STATS_T, OPERATION_STATS_W, tft.getFontHeight(), ILI9341_BLACK);
  tft.fillRect(OPERATION_STATS_L, OPERATION_STATS_T, OPERATION_STATS_W, 10, ILI9341_BLACK);
...
FLASHMEM void ShowCurrentPowerSetting() {
  ...
  //tft.fillRect(OPERATION_STATS_PWR, OPERATION_STATS_T, tft.getFontWidth() * 11, tft.getFontHeight(), ILI9341_BLACK);
  tft.fillRect(OPERATION_STATS_PWR, OPERATION_STATS_T, 6 * 11, 8, ILI9341_BLACK);
