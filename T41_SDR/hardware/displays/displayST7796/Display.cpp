...
FASTRUN void ShowFrequency() {
  ...
  //tft.fillRect(FREQUENCY_X, FREQUENCY_Y, SPEC_BOX_W, tft.getFontHeight(), ST7735_BLACK);
  tft.fillRect(FREQUENCY_X, FREQUENCY_Y, SPEC_BOX_W, 20*8, ST7735_BLACK);

  if(activeVFO == VFO_A) {
    if(TxRxFreq < bands[currentBandA].fBandLow || TxRxFreq > bands[currentBandA].fBandHigh) {
      tft.setTextColor(ST7735_RED);  // Out of band
    } else {
      tft.setTextColor(ST7735_GREEN); // In US band
    }
